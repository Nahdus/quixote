 ...
 */
  return {
    respond: (phrase, fileName, threshold) =>
      respond(phrase, fileName, threshold)
        .then(data => {
          switch (state.task) {
            case "identifyIntent":
              if (JSON.parse(data.srcAnswer).hasOwnProperty("text")) {
                ...
                return new Promise(resolve =>
                  resolve(
                    JSON.parse(data.srcAnswer).text[
                      chooseRand(JSON.parse(data.srcAnswer).text)
                    ]
                  )
                );
              ...
              if (memory.hasOwnProperty("phoneNumber")) {
                ...
                if (action.ifGot.hasOwnProperty("text")) {
                  response = new Promise(resolve =>
                    resolve(action.ifGot.text[chooseRand(action.ifGot.text)])
                  );
                }
                ...
                return new Promise(resolve =>
                  resolve(
                    action.ifGotNot.text[chooseRand(action.ifGotNot.text)]
                  )
                );
              }
            case "email":
